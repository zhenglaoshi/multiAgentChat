...
**新增**
- **代码改动强制评审门（软性/指南级）**：任意 shell 写完/改完产品代码，交付前**必须**用 Task 并行派 `code-reviewer` + `security-reviewer` 审 `git diff`，有 high/critical 就按建议改、再复审到通过才算完成（纯文档/配置豁免）。落两处:① 全局 `~/.claude/CLAUDE.md`(覆盖用户**任意 shell** 的每个 claude session) ② 本项目 `SYSTEM_GUIDANCE`(飞书派发任务注入,version-controlled)。软性=强约定非物理拦截(用户选 A;硬性 Stop-hook 拦截留作可选)。（`im-lark/lark/handlers.ts` `SYSTEM_GUIDANCE` + `~/.claude/CLAUDE.md`)
