...
export const INTEGRATIONS: Integration[] = [
  ...
  },
  {
    ...
    claudeMdRule: [
      ...
      "",
      "1. **首选 `agent bootcheck`**（本机通用机器门，任意项目可用，不依赖 daemon）：自动发现该项目自己的",
      "   验证命令（`typecheck` → `check:boot`/`check:schema`/`smoke` → 退化 `build`）并跑完，输出 pass/fail",
      "   与每步 exit code；`--json` 可直接贴进交付说明。工具不在或非 Node 项目 → 手动按第 2 条。",
      "2. 手动兜底：从 `package.json` / `Makefile` / CI 里读项目真实命令（别编），typecheck / build / lint / test 有哪个跑哪个。",
      "3. **加载期冒烟**：让改动过的模块真的被编译 / import / 组装一次",
      "   （起服务看是否监听、构造 schema 打印字段数、`node -e \"require('./dist/x')\"` 均可）。",
      "   `agent bootcheck` 的 `hasBootSmoke: false` 就是在说「这仓库没有这一步」——那就补一个 `check:boot`，",
      "   或在交付里如实写明缺这一环。",
      "4. **跑之前先看这条命令会碰什么**：会不会连真实 DB / MQ / Redis / 第三方（尤其本机 env 里已配了",
      ...
      "5. 跑不动（缺依赖 / 要连库 / 要凭证 / 没这命令）→ 交付时**如实写「未做机器验证：原因」**，不许默默省略。",
      "",
