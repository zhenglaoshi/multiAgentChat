...
### 命令（按需）
```
/report day            # 今日简报（不给日期 = 当天）
/report week           # 本周简报（默认周期）
/report month          # 本月 PPT（--brief 出简报）
/report year           # 本年 PPT

# 补历史：加一个日期锚点（周期取「含这天的那一整个周期」）
/report 昨天            # 昨天的日报（相对日隐含 day）
/report day 2026-09-01  # 指定那天的日报
/report 09-01           # 省略年份 = 当年
/report 上周五          # 周N 写法（周三/本周三/上星期一/周天…）
/report 8月             # 裸月份 = 那个月的月报；/report 3日 = 那天的日报
/report 3天前 / -3      # N 天前
/report week 上周       # 上周周报；/report week 2026-08-20 = 含那天的那一周
/report 2026-08         # 2026 年 8 月月报（YYYY-MM 隐含 month）
/report 2026 --brief    # 2026 年报，出简报而非 PPT
```
锚点日期按**北京时区日历日**解析；给未来日期会直接拒绝。**省略年份/月份的写法**（`09-01`/`8月`/`30日`）
取「**最近一次已发生**」的那个——9 月里说「12月」指去年 12 月、7 号说「30日」指上月 30 号，
落定的窗口会在「生成中」那条消息里回显（`📊 月报（2025-12-01~2025-12-31）`）；带全年份的写法不回退。周期关键词（day/week/month/year）优先于
日期写法隐含的周期；日期缺省时行为与原来完全一致（当天/当期）。历史周期的时间窗是**整段闭合**的
（如 09-01 的日报 = 09-01 00:00~09-02 00:00），当期则收到「现在」。
